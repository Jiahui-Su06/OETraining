...
SAVE_DIR = "output/results/PCA_Ridge"
...
def load_all_models():
    ...
    for comp in COMPONENTS:
        try:
            models[comp] = joblib.load(f"{MODEL_DIR}/{comp}_ridge.joblib")
            scalers[comp] = joblib.load(f"{MODEL_DIR}/{comp}_scaler.joblib")
            indices[comp] = joblib.load(f"{MODEL_DIR}/{comp}_wavelength_indices.joblib")
            print(f"Loaded {comp} model successfully")
        except Exception as e:
            print(f"ERROR loading {comp}: {e}")
            return None, None, None
    return models, scalers, indices
...
def plot_predictions(pred_df, actual_df, save_path):
    ...
    for i, comp in enumerate(COMPONENTS):
        ...
        ax.set_title(f"{comp} Content - Ridge Test Prediction")
...
def main():
    ...
    try:
        ...
        pred_df.to_excel(f"{SAVE_DIR}/test_predictions.xlsx", index=False)
        plot_predictions(pred_df, test_actual, f"{SAVE_DIR}/test_plot.png")
